...
import Dibujos.Grillanum (grillanumConf)
...
configs :: [Conf]
-- ACA SE AGREGA EL CONF DE UN NUEVO DIBUJO
--configs = [ejemploConf, feoConf,cuadConf 3]
configs = [feoConf, grillanumConf, escherConf] 
...
main = do
  ...
  when (head args == "-l") $ do
    ...
    putStrLn "Cual desea mostrar?:"    
    draw <- getLine
    initial' configs $ draw
    exitSuccess
